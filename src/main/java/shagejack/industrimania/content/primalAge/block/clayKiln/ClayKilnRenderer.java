...
public class ClayKilnRenderer extends SafeTileEntityRenderer<ClayKilnTileEntity> {

    public ClayKilnRenderer(BlockEntityRendererProvider.Context context) {}

    @Override
    protected void renderSafe(ClayKilnTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        if (!te.getBlockState().isAir())
            renderItems(te, partialTicks, ms, buffer, light, overlay);
    }

    protected void renderItems(ClayKilnTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
