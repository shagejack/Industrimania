...
public abstract class AbstractSkinnedCarcass extends Block {
    ...
    public AbstractSkinnedCarcass(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (player.getItemInHand(handIn).getItem() instanceof Saw) {
            if (level.isClientSide)
                return InteractionResult.SUCCESS;

            dropItem(level, pos, onCut(level, pos));
        }
    ...
    public abstract ItemStack onCut(Level level, BlockPos pos);
