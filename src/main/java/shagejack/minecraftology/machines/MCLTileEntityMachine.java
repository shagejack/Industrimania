...
import shagejack.minecraftology.client.sound.MachineSound;
import shagejack.minecraftology.data.Inventory;
import shagejack.minecraftology.data.TileEntityInventory;
import shagejack.minecraftology.fx.VentParticle;
import shagejack.minecraftology.machines.components.ComponentConfigs;
import shagejack.minecraftology.machines.configs.ConfigPropertyStringList;
...
import shagejack.minecraftology.util.math.MCLMathHelper;
 ...
 * @since 3/11/2015
 */
public abstract class MCLTileEntityMachine extends MCLTileEntity implements IMCLTileEntity, ISidedInventory, ITickable {
    ...
    protected void registerComponents() {
        ...
        configs.addProperty(new ConfigPropertyStringList(
                ...
                "gui.config.redstone",
                new String[]{
                        MCLStringHelper.translateToLocal("gui.redstone_mode.low"),
                        MCLStringHelper.translateToLocal("gui.redstone_mode.high"),
                        MCLStringHelper.translateToLocal("gui.redstone_mode.disabled")},
                0
        ));
        ...
        if (hasSound() && soundVolume > 0) {
            if (isActive() && !isInvalid()) {
                if (sound == null) {
                    ...
                    if (getBlockType() instanceof MCLBlockMachine) {
                        soundMultiply = ((MCLBlockMachine) getBlockType()).volume;
                    }
                    if (soundMultiply > 0) {
        ...
        if (categories.contains(MachineNBTCategory.DATA)) {
            ...
            if (nbt.hasKey("Owner", 8) && !nbt.getString("Owner").isEmpty()) {
                try {
                    ...
                    LogMCL.log(Level.ERROR, "Invalid Owner ID: " + nbt.getString("Owner"));
                }
            }
        }
        ...
        if (itemStack.hasTagCompound()) {
            ...
            if (machineTag.hasKey("Owner", 8)) {
                try {
                    ...
                    LogMCL.log(Level.ERROR, e, "Invalid Owner ID: " + machineTag.getString("Owner"));
                }
            }
        }
    }
    ...
    public void SpawnVentParticles(float speed, EnumFacing side, int count) {
        for (int i = 0; i < count; i++) {
            ...
            Vector3f circle = MCLMathHelper.randomCirclePoint(random.nextFloat(), random);
            circle.scale(0.4f);
    ...
    public <T extends MCLBlock> T getBlockType(Class<T> type) {
