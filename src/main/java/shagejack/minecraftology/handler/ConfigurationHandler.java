...
import shagejack.minecraftology.util.IConfigSubscriber;
...
public class ConfigurationHandler {
    public static final String CATEGORY_MCL = "minecraftology registry";
    public static final String CATEGORY_MCL_ITEMS = "minecraftology items registry";
    ...
    public void init() {
        config.load();
        ConfigCategory category = config.getCategory(CATEGORY_MCL);
        category.setComment("Configuration for the Matter ");
        updateCategoryLang(category);
        config.get(CATEGORY_MCL, KEY_MBLACKLIST, new String[0]).setComment("Blacklist for items in the matter registry. Automatic Recipe calculation will ignore recipes with these items. Just add the unlocalized name or the ore dictionary name in the list.");
        config.get(CATEGORY_MCL, KEY_BLACKLIST_MODS, new String[0]).setComment("Blacklist for mods (mod ID). Automatic Recipe calculation will ignore recipes with items from this mod");
        category = config.getCategory(CATEGORY_MCL_ITEMS);
