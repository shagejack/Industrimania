...
public class ElementProgressBar extends MCLElementBase {
    ...
    public ElementProgressBar(MCLGuiBase gui, int posX, int posY) {
        super(gui, posX, posY);
    ...
    public void drawBackground(int mouseX, int mouseY, float gameTicks) {
        ...
        for (int i = 0; i < 26; i++) {
            ...
            if (i < progress) {
                double noiseValue = ((MCLMathHelper.noise(0, 0.05f * i, Math.pow(seed * 100, 2)) + 1.0) / 2.0);
            ...
            values[i] = MCLMathHelper.Lerp(values[i], newValue, 0.05f);
        }
