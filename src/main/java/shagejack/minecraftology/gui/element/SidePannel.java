package shagejack.minecraftology.gui.element;

import net.minecraft.client.renderer.GlStateManager;
import net.minecraft.util.ResourceLocation;
import shagejack.minecraftology.Reference;
import shagejack.minecraftology.data.ScaleTexture;
import shagejack.minecraftology.gui.MCLGuiBase;
import shagejack.minecraftology.util.MCLStringHelper;

public class SidePannel extends ElementBaseGroup {
    private static boolean isOpen;
    public final ScaleTexture BACKGROUND_TEXTURE = new ScaleTexture(new ResourceLocation(Reference.PATH_ELEMENTS + "right_side_bar_panel_bg.png"), 15, 18).setOffsets(7, 7, 8, 9);
    MCLElementButton button;
    private boolean openable;

    public SidePannel(MCLGuiBase gui, int posX, int posY, int height) {
        this(gui, posX, posY, height, true);
    }

    public SidePannel(MCLGuiBase gui, int posX, int posY, int height, boolean openable) {
        super(gui, posX, posY, 37, height);
        MCLElementButtonScaled button = new MCLElementButtonScaled(gui, this, 0, 0, "Toggle", 16, height);
        button.setNormalTexture(new ScaleTexture(new ResourceLocation(Reference.PATH_ELEMENTS + "right_side_bar_over.png"), 32, 143).setOffsets(0, 0, 42, 100));
        button.setOverTexture(new ScaleTexture(new ResourceLocation(Reference.PATH_ELEMENTS + "right_side_bar_over.png"), 32, 143).setOffsets(0, 0, 42, 100));
        //button.setSounds(MCLSounds.guiButtonExpand.getSoundName());
        this.button = button;
        this.openable = openable;

    }

    @Override
    public void init() {
        super.init();
        if (button != null) {
            elements.add(button);
        }
    }

    @Override
    public void drawBackground(int mouseX, int mouseY, float ticks) {
        if (openable) {
            if (isOpen) {
                GlStateManager.color(1, 1, 1);
                BACKGROUND_TEXTURE.render(posX, posY, 37, sizeY);
                button.setPosition(32, 0);
            } else {
                button.setPosition(0, 0);
            }
        }

        super.drawBackground(mouseX, mouseY, ticks);
    }

    public boolean isOpen() {
        return isOpen;
    }

    public void setOpen(boolean open) {
        isOpen = open;
    }

    @Override
    public void handleElementButtonClick(MCLElementBase element, String buttonName, int mouseButton) {
        if (buttonName.equals("Toggle")) {
            isOpen = !isOpen;
        }
    }

    @Override
    public void update() {
        if (openable) {
            if (isOpen) {
                button.setToolTip(MCLStringHelper.translateToLocal("gui.tooltip.close_menu"));
                this.setSize(37 + 16, sizeY);
            } else {
                button.setToolTip(MCLStringHelper.translateToLocal("gui.tooltip.open_menu"));
                this.setSize(37, sizeY);
            }
        }
    }

    @Override
    public void updateInfo() {
        for (MCLElementBase element : elements) {
            element.setVisible(isOpen);
        }

        button.setVisible(openable);
    }

    public void setOpenable(boolean openable) {
        this.openable = openable;
    }
}

