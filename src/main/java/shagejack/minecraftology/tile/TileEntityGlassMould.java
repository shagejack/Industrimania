...
import shagejack.minecraftology.data.tank.GlassMeltingFurnaceTank;
...
public class TileEntityGlassMould extends MCLTileEntityMachine implements ITickable {
    ...
    public void update() {
        ...
        if (!getWorld().isRemote) {
            if (getBlowCount() >= 20 && tank.getFluidAmount() >= Fluid.BUCKET_VOLUME) {
                ...
                ItemStack recipes[] = {
                        new ItemStack(Minecraftology.ITEMS.glass_mould_block)
                };
                ...
                if(recipe != -1) {
                    if (!tank.getFluid().isFluidEqual(new FluidStack(FluidsMCL.halfMoltenGlass, Fluid.BUCKET_VOLUME))) recipe = -1;
                }
