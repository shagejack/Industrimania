...
import shagejack.minecraftology.api.wrench.IDismantleable;
...
import shagejack.minecraftology.util.*;
...
public abstract class MCLBlockMachine<TE extends TileEntity> extends MCLBlockContainer<TE> implements IDismantleable {
    public float volume = 1;
    public boolean hasGui;

    public MCLBlockMachine(Material material, String name) {
        super(material, name);
    }
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        IMCLTileEntity entity = (IMCLTileEntity) worldIn.getTileEntity(pos);
        if (entity != null) {
            try {
                entity.readFromPlaceItem(stack);
            } catch (Exception e) {
                e.printStackTrace();
                LogMCL.log(Level.ERROR, "Could not load settings from placing item", e);
            }
    ...
    public ItemStack getNBTDrop(World world, BlockPos blockPos, IMCLTileEntity te) {
    ...
    public ArrayList<ItemStack> dismantleBlock(EntityPlayer player, World world, BlockPos pos, boolean returnDrops) {
        ...
        ItemStack blockItem = getNBTDrop(world, pos, (IMCLTileEntity) world.getTileEntity(pos));
        ...
        if (!returnDrops) {
            ...
            MCLInventoryHelper.insertItemStackIntoInventory(player.inventory, blockItem, EnumFacing.DOWN);
        }
    ...
    protected Inventory getInventory(World world, BlockPos pos) {
        if (world.getTileEntity(pos) instanceof MCLTileEntityMachine) {
            MCLTileEntityMachine machine = (MCLTileEntityMachine) world.getTileEntity(pos);
    ...
    public boolean canDismantle(EntityPlayer player, World world, BlockPos pos) {
        ...
        if (tileEntity instanceof MCLTileEntityMachine) {
            if (player.capabilities.isCreativeMode || !((MCLTileEntityMachine) tileEntity).hasOwner()) {
                return true;
            } else {
                if (((MCLTileEntityMachine) tileEntity).getOwner().equals(player.getGameProfile().getId())) {
                    return true;
                } else {
                    if (world.isRemote) {
