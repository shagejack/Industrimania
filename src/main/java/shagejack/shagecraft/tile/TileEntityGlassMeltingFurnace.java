...
import shagejack.shagecraft.api.events.glassmeltingfurnace.ShageEventGlassMeltingFurnaceConsume;
import shagejack.shagecraft.data.Inventory;
import shagejack.shagecraft.data.inventory.GlassMeltingFurnaceSlot;
...
public class TileEntityGlassMeltingFurnace extends ShageTileEntityMachine implements IShageTickable, ITickable {
    ...
    private final String input = "shagecraft:mechanic.glass_melting_furnace_input";
    private final String output = "shagecraft:mechanic.glass_melting_furnace_output";
    ...
    public static boolean isMaterial(ItemStack stack){
        if(stack.getItem() == Shagecraft.ITEMS.raw_glass_material) return true;
        return false;
    }
    ...
    public boolean isBurning(){
       TileEntityGlassMeltingFurnaceInput tileEntity = (TileEntityGlassMeltingFurnaceInput) world.getTileEntity(pos.add(ShageMultiBlockCheckHelper.getRotatedPos(new BlockPos(0, -1, 1), complete)));
    ...
    private boolean consumeEntityItem(EntityItem entityItem) {
        ...
        if (!itemStack.isEmpty()) {
            ...
            if (itemStack.getItem() == Shagecraft.ITEMS.raw_glass_material) {
                if (slot.getItem().isEmpty()){
                    inventory.setInventorySlotContents(input_slot, itemStack);
                    entityItem.setDead();
                    world.removeEntity(entityItem);
                }
                if (slot.getItem().getItem() == Shagecraft.ITEMS.raw_glass_material){
    ...
    private boolean onEntityConsume(Entity entity, boolean pre) {
        if (pre) {
            MinecraftForge.EVENT_BUS.post(new ShageEventGlassMeltingFurnaceConsume.Pre(entity, getPos()));
        } else {
            MinecraftForge.EVENT_BUS.post(new ShageEventGlassMeltingFurnaceConsume.Post(entity, getPos()));
        }
    ...
    public void manageOutput(){
        if (temperature > 1473.15) {
            TileEntityGlassMeltingFurnaceOutput tileEntity = (TileEntityGlassMeltingFurnaceOutput) world.getTileEntity(pos.add(ShageMultiBlockCheckHelper.getRotatedPos(new BlockPos(0, 0, -1), complete)));
            if (tileEntity == null) return;

            ItemStack inputStack = inventory.getStackInSlot(input_slot);
            if (inputStack.getItem() == Shagecraft.ITEMS.raw_glass_material){
                if (idle <= 0){
                    inputStack.shrink(1);
                    if (tileEntity.tank.canFillFluidType(new FluidStack(ShagecraftFluids.halfMoltenGlass, 100)) && tileEntity.tank.fill(new FluidStack(ShagecraftFluids.halfMoltenGlass, 100), false) == 100){
                        tileEntity.tank.fill(new FluidStack(ShagecraftFluids.halfMoltenGlass, 100), true);
                        idle = 1200;
                    }
                } else {
                    idle --;
