package shagejack.shagecraft.machines;
...
import shagejack.shagecraft.api.container.IMachineWatcher;
import shagejack.shagecraft.blocks.includes.ShageBlock;
import shagejack.shagecraft.blocks.includes.ShageBlockMachine;
import shagejack.shagecraft.client.sound.MachineSound;
import shagejack.shagecraft.data.Inventory;
import shagejack.shagecraft.data.TileEntityInventory;
import shagejack.shagecraft.fx.VentParticle;
import shagejack.shagecraft.machines.components.ComponentConfigs;
import shagejack.shagecraft.machines.configs.ConfigPropertyStringList;
...
import shagejack.shagecraft.util.math.ShageMathHelper;
 ...
 * @since 3/11/2015
 */
public abstract class ShageTileEntityMachine extends ShageTileEntity implements IShageTileEntity, ISidedInventory, ITickable {
    ...
    public void update() {
        ...
        components.stream().filter(component -> component instanceof ITickable).forEach(component -> {
            try {
                ...
                LogShage.log(Level.FATAL, e, "There was a problem while ticking %s component %s", this, component);
            }
        });
    }
    ...
    protected void registerComponents() {
        ...
        configs.addProperty(new ConfigPropertyStringList(
                ...
                "gui.config.redstone",
                new String[]{
                        ShageStringHelper.translateToLocal("gui.redstone_mode.low"),
                        ShageStringHelper.translateToLocal("gui.redstone_mode.high"),
                        ShageStringHelper.translateToLocal("gui.redstone_mode.disabled")},
                0
        ));
        ...
        if (hasSound() && soundVolume > 0) {
            if (isActive() && !isInvalid()) {
                if (sound == null) {
                    ...
                    if (getBlockType() instanceof ShageBlockMachine) {
                        soundMultiply = ((ShageBlockMachine) getBlockType()).volume;
                    }
                    if (soundMultiply > 0) {
        ...
        if (categories.contains(MachineNBTCategory.DATA)) {
            ...
            if (nbt.hasKey("Owner", 8) && !nbt.getString("Owner").isEmpty()) {
                try {
                    ...
                    LogShage.log(Level.ERROR, "Invalid Owner ID: " + nbt.getString("Owner"));
                }
            }
        }
        ...
        if (itemStack.hasTagCompound()) {
            ...
            if (machineTag.hasKey("Owner", 8)) {
                try {
                    ...
                    LogShage.log(Level.ERROR, e, "Invalid Owner ID: " + machineTag.getString("Owner"));
                }
            }
        }
    }
    ...
    protected void manageClientSync() {
        if (forceClientUpdate) {
            ...
            Shagecraft.NETWORK.sendToAllAround(new PacketSendMachineNBT(MachineNBTCategory.ALL_OPTS, this, false, false), this, 64);
            markDirty();
        }
    }
    ...
    public void SpawnVentParticles(float speed, EnumFacing side, int count) {
        for (int i = 0; i < count; i++) {
            ...
            Vector3f circle = ShageMathHelper.randomCirclePoint(random.nextFloat(), random);
            circle.scale(0.4f);
    ...
    public <T extends ShageBlock> T getBlockType(Class<T> type) {
