...
package shagejack.shagecraft.data.transport;

import net.minecraft.block.state.IBlockState;
import net.minecraft.tileentity.TileEntity;
import net.minecraft.util.EnumFacing;
import net.minecraft.util.math.BlockPos;
import shagejack.shagecraft.api.transport.IGridNetwork;
import shagejack.shagecraft.api.transport.IGridNode;
import shagejack.shagecraft.handler.steam_network.GridNetworkHandler;
...
public abstract class AbstractGridNetwork<T extends IGridNode> implements IGridNetwork<T> {
    ...
    public void addNode(T node) {
        if (node.getNetwork() != null) {
            if (node.getNetwork() != this) {
                ...
                LogShage.info("Adding Node %s to it's own network %s", node, this);
            }
        } else {
            nodes.add(node);
    ...
    public void removeNode(T node) {
        if (node.getNetwork() == null || node.getNetwork() == this) {
            ...
            LogShage.info("Trying to remove node from a different network than it's assigned one");
        }
    }
