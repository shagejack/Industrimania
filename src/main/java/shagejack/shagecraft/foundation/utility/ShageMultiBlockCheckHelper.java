...
public class ShageMultiBlockCheckHelper {
    ...
    public static int checkComplete(Level world, BlockPos pos, String[] structure, BlockPos[] rPos, boolean debug) {
        ...
        for (int i = 0; i < structure.length; i++) {
                if (!world.getBlockState(pos.offset(getRotatedPos(rPos[i], j))).getBlock().getRegistryName().toString().equalsIgnoreCase(structure[i])) {
                    complete[j] = false;
                    if(debug) ShageCraft.LOGGER.debug("Rotation Type [" + j + "] PosArr" + rPos[i] + " Wrong Block on" + pos.offset(getRotatedPos(rPos[i], j)) + " which should be " + structure[i] + " but actually " + world.getBlockState(pos.offset(getRotatedPos(rPos[i], j))).getBlock().getRegistryName().toString());
                }
            }
        }
        ...
        for(int i = 0; i < complete.length; i++){
            if (complete[i]) {
                if(debug) ShageCraft.LOGGER.debug("Complete! Rotation Type = " + i);
                return i;
            }
            }

        if(debug) ShageCraft.LOGGER.debug("Incomplete! Rotation Type = -1");
        return -1;
