    ...
    public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, EnumHand hand) {
        ...
        if (!state.getBlock().isAir(state, world, pos)) {
            ...
            if (te != null && te.hasCapability(ShagecraftCapabilities.STEAM_HANDLER, null)) {
                ...
                String msg = "\u5bb9\u91cf: " + steam_occupied_capacity + " / " + steam_capacity + " (" + (steam_occupied_capacity / steam_capacity * 100) + "%)";
                if(!player.isSneaking()) {
                    if (world.isRemote) {
                        player.sendMessage(new TextComponentString("\u84b8\u6c7d\u8d28\u91cf: " + steam_properties[0] + " KG"));
                        if (steam_properties[2] == 0) {
                            player.sendMessage(new TextComponentString("\u84b8\u6c7d\u6e29\u5ea6: N/A"));
                        } else {
