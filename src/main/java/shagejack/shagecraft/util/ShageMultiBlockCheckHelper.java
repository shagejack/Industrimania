...
public class ShageMultiBlockCheckHelper {
    ...
    public static int checkComplete(World world, BlockPos pos, String[] structure, BlockPos[] rPos, boolean debug) {
        ...
        for (int i = 0; i < structure.length; i++) {
                if (!world.getBlockState(pos.add(getRotatedPos(rPos[i], j))).getBlock().getRegistryName().toString().equalsIgnoreCase(structure[i])) {
                    complete[j] = false;
                    if(debug) LogShage.debug("Rotation Type [" + j + "] PosArr" + rPos[i] + " Wrong Block on" + pos.add(getRotatedPos(rPos[i], j)) + " which should be " + structure[i] + " but actually " + world.getBlockState(pos.add(getRotatedPos(rPos[i], j))).getBlock().getRegistryName().toString());
                }
            }
        }
        ...
        for(int i = 0; i < complete.length; i++){
            if (complete[i]) {
                if(debug) LogShage.debug("Complete! Rotation Type = " + i);
                return i;
            }
            }

        if(debug) LogShage.debug("Incomplete! Rotation Type = -1");
        return -1;
