...
import shagejack.shagecraft.util.ShageInventoryHelper;

import javax.annotation.Nonnull;

public class ContainerMachine<T extends ShageTileEntityMachine> extends ShageBaseContainer implements IMachineWatcher {
    protected T machine;
    ...
    public void addAllSlotsFromInventory(Inventory inventory) {
        for (shagejack.shagecraft.data.inventory.Slot slot : inventory.getSlots()) {
    ...
    protected boolean putInPlayerInventory(ItemStack itemStack) {
        return ShageInventoryHelper.mergeItemStack(inventorySlots, itemStack, machine.getSizeInventory(), inventorySlots.size() - machine.getSizeInventory(), true, true);
    }

    protected boolean tryAndPutInMachineSlots(ItemStack itemStack, IInventory inventory) {
        return ShageInventoryHelper.mergeItemStack(inventorySlots, itemStack, 0, inventory.getSizeInventory(), false, true);
    }
    ...
    @Override
    public void onWatcherAdded(ShageTileEntityMachine machine) {
