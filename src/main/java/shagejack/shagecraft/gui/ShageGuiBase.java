...
import shagejack.shagecraft.data.ScaleTexture;
import shagejack.shagecraft.gui.element.*;
import shagejack.shagecraft.gui.events.IListHandler;
import shagejack.shagecraft.gui.events.ITextHandler;
...
public abstract class ShageGuiBase extends GuiContainer implements IButtonHandler, ITextHandler, IListHandler, GuiElementList {
    ...
    protected List<ShageElementButton> pageButtons;
    protected ArrayList<ShageElementBase> elements;
    ...
    public ShageGuiBase(ShageBaseContainer container) {
        this(container, 225, 186);

    }

    public ShageGuiBase(ShageBaseContainer container, int width, int height) {
        super(container);
    ...
    public void initGui() {
        ...
        for (ShageElementButton button : pageButtons) {
    ...
    public void addTooltips(List<String> list) {
        for (int i = elements.size(); i-- > 0; ) {
            ShageElementBase element = elements.get(i);
            if (element instanceof ShageElementBase && element.isVisible() && element.intersectsWith(mouseX, mouseY)) {
    ...
    @Override
    public void handleElementButtonClick(ShageElementBase element, String buttonName, int mouseButton) {
    ...
    public void handleMouseInput() throws IOException {
        ...
        if (var3 != 0) {
            ...
            while (var4-- > 0) {
                ShageElementBase var5 = this.elements.get(var4);
    ...
    protected void updateElementInformation() {
        ...
        for (int i = elements.size(); i-- > 0; ) {
            if (elements.get(i) instanceof ShageElementBase) {
    ...
    protected final void updateElements(float partialTicks) {
        ...
        while (elementCount-- > 0) {
            ShageElementBase element = this.elements.get(elementCount);
    ...
    public void registerPages(ShageBaseContainer container) {
    }
    ...
    protected ShageElementBase getElementAtPosition(int mX, int mY) {
    ...
    public ShageElementIconButton AddPage(ElementBaseGroup page, HoloIcon icon, String tooltip) {
        pages.add(page);
        ShageElementIconButton button = new ShageElementIconButton(this, this, 0, 0, page.getName(), 0, 0, 24, 0, 24, 0, 24, 24, "", icon);
    ...
    public ShageBaseContainer getContainer() {
        return (ShageBaseContainer) inventorySlots;
    }

    @Override
    public ShageElementBase addElement(ShageElementBase element) {
    ...
    public List<ShageElementBase> getElements() {
        return elements;
    }
