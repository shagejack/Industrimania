...
public class SidePannel extends ElementBaseGroup {
    ...
    ShageElementButton button;
    private boolean openable;

    public SidePannel(ShageGuiBase gui, int posX, int posY, int height) {
        this(gui, posX, posY, height, true);
    }

    public SidePannel(ShageGuiBase gui, int posX, int posY, int height, boolean openable) {
        super(gui, posX, posY, 37, height);
        ShageElementButtonScaled button = new ShageElementButtonScaled(gui, this, 0, 0, "Toggle", 16, height);
        ...
        //button.setSounds(MCLSounds.guiButtonExpand.getSoundName());
        this.button = button;
    ...
    public void update() {
        if (openable) {
            if (isOpen) {
                button.setToolTip(ShageStringHelper.translateToLocal("gui.tooltip.close_menu"));
                this.setSize(37 + 16, sizeY);
            } else {
                button.setToolTip(ShageStringHelper.translateToLocal("gui.tooltip.open_menu"));
    ...
    public void updateInfo() {
        for (ShageElementBase element : elements) {
