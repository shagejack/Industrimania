...
public class ElementProgressBar extends ShageElementBase {
    ...
    public ElementProgressBar(ShageGuiBase gui, int posX, int posY) {
        super(gui, posX, posY);
    ...
    public void drawBackground(int mouseX, int mouseY, float gameTicks) {
        ...
        for (int i = 0; i < 26; i++) {
            ...
            if (i < progress) {
                double noiseValue = ((ShageMathHelper.noise(0, 0.05f * i, Math.pow(seed * 100, 2)) + 1.0) / 2.0);
            ...
            values[i] = ShageMathHelper.Lerp(values[i], newValue, 0.05f);
        }
